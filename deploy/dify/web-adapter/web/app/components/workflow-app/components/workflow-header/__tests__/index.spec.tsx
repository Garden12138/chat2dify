...
vi.mock('@/app/components/chat2dify/workflow-trigger', () => ({
  default: ({ appId, appMode, appName }: Record<string, string | undefined>) => (
    <div
      data-testid="chat2dify-workflow-trigger"
      data-app-id={appId}
...
describe('WorkflowHeader', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render Chat2Dify workflow trigger with current app context', () => {
      // Arrange
      setupAppStore({ mode: AppModeEnum.WORKFLOW, name: '售后工作流' })

      // Act
      render(<WorkflowHeader />)

      // Assert
      const trigger = screen.getByTestId('chat2dify-workflow-trigger')
      expect(trigger).toHaveAttribute('data-app-id', 'app-id')
      expect(trigger).toHaveAttribute('data-app-mode', 'workflow')
      expect(trigger).toHaveAttribute('data-app-name', '售后工作流')
    })
  })
