...
vi.mock('@/app/components/chat2dify/panel', () => ({
  default: ({ open, intent, appMode, onOpenChange }: Record<string, unknown>) => {
    if (!open)
      return null
    ...
    return React.createElement(
      'div',
      {
        'data-testid': 'chat2dify-panel',
        'data-intent': intent as string,
        'data-app-mode': appMode as string,
      },
      React.createElement('button', { 'onClick': () => (onOpenChange as (open: boolean) => void)(false), 'data-testid': 'close-chat2dify-panel' }, 'Close'),
    )
  },
}))
...
describe('CreateAppCard', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render four create buttons', () => {
      ...
      expect(screen.getByText('Chat2Dify 创建')).toBeInTheDocument()
  ...
  describe('User Interactions - Chat2Dify Panel', () => {
    it('should open Chat2Dify panel when clicking Chat2Dify create', () => {
      render(<CreateAppCard ref={defaultRef} selectedAppType="advanced-chat" />)

      fireEvent.click(screen.getByText('Chat2Dify 创建'))

      const panel = screen.getByTestId('chat2dify-panel')
      expect(panel).toHaveAttribute('data-intent', 'create')
      expect(panel).toHaveAttribute('data-app-mode', 'advanced-chat')
    })

    it('should close Chat2Dify panel', () => {
      render(<CreateAppCard ref={defaultRef} />)

      fireEvent.click(screen.getByText('Chat2Dify 创建'))
      expect(screen.getByTestId('chat2dify-panel')).toBeInTheDocument()

      fireEvent.click(screen.getByTestId('close-chat2dify-panel'))
      expect(screen.queryByTestId('chat2dify-panel')).not.toBeInTheDocument()
    })
  })
