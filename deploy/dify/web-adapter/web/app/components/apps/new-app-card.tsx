...
import Chat2DifyPanel from '@/app/components/chat2dify/panel'
...
const CreateAppCard = ({
  ...
  const [showChat2DifyPanel, setShowChat2DifyPanel] = useState(false)
  ...
  const chat2DifyAppMode = selectedAppType && selectedAppType !== 'all' ? selectedAppType : undefined
  ...
  return (
    ...
    >
      <div className="grow rounded-t-xl p-2">
        ...
        <button type="button" className="flex w-full cursor-pointer items-center rounded-lg px-6 py-[5px] text-[13px] leading-[18px] font-medium text-text-tertiary hover:bg-state-base-hover hover:text-text-secondary" onClick={() => setShowNewAppModal(true)}>
          <FilePlus01 className="mr-2 size-4 shrink-0" />
          {t('newApp.startFromBlank', { ns: 'app' })}
        </button>
        <button type="button" className="flex w-full cursor-pointer items-center rounded-lg px-6 py-[5px] text-[13px] leading-[18px] font-medium text-text-tertiary hover:bg-state-base-hover hover:text-text-secondary" onClick={() => setShowNewAppTemplateDialog(true)}>
          <FilePlus02 className="mr-2 size-4 shrink-0" />
          {t('newApp.startFromTemplate', { ns: 'app' })}
        </button>
        <button
          type="button"
          onClick={() => setShowChat2DifyPanel(true)}
          className="flex w-full cursor-pointer items-center rounded-lg px-6 py-[5px] text-[13px] leading-[18px] font-medium text-text-tertiary hover:bg-state-base-hover hover:text-text-secondary"
        >
          <span aria-hidden className="i-ri-sparkling-line mr-2 size-4 shrink-0" />
          Chat2Dify 创建
        </button>
        <button
          type="button"
      ...
      <Chat2DifyPanel
        open={showChat2DifyPanel}
        intent="create"
        appMode={chat2DifyAppMode}
        onOpenChange={setShowChat2DifyPanel}
      />
