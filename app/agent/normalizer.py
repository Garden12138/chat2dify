...
def normalize_plan_payload(
    ...
    *,
    ...
    if app_description and data.get("description") != app_description:
        data["description"] = app_description
        changes.append("set workflow description from request app_description")
    target_app_mode = str(app_mode or data.get("app_mode") or "workflow")
    if target_app_mode not in {"workflow", "advanced-chat", "agent-chat"}:
        target_app_mode = "workflow"
    if data.get("app_mode") != target_app_mode:
    ...
    for node in nodes:
        ...
        match node_type:
            case "start":
                node["params"] = _normalize_start_params(params, app_mode=target_app_mode)
            case "llm":
                node["params"] = _normalize_llm_params(
                    params,
                    workflow_name=str(data.get("name", "")),
                    app_mode=target_app_mode,
                )
            case "code":
            ...
            case "answer":
                node["params"] = _normalize_answer_params(params)
            case "http-request":
    ...
    if target_app_mode == "advanced-chat":
        ...
        for node in nodes:
            ...
            rewritten, rewrite_count = _rewrite_chatflow_input_references(
                params,
                start_ids=start_ids,
            )
            if rewrite_count:
...
def _normalize_chatflow_terminal_nodes(
    nodes: list[Any],
    changes: list[str],
) -> None:
    end_nodes = [
    ...
    *,
    ...
    if isinstance(value, str):
        ...
        def replace(match: re.Match[str]) -> str:
            ...
            if node_id not in start_ids or variable not in {
                "query",
                "files",
                "sys.query",
                "sys.files",
            ...
            count += 1
            system_variable = variable.removeprefix("sys.")
            return f"{{{{#sys.{system_variable}#}}}}"
    ...
    if isinstance(value, list):
        if (
            ...
            and value[0] in start_ids
            and value[1] in {"query", "files"}
        ):
            return [value[0], f"sys.{value[1]}"], 1
        result: list[Any] = []
        count = 0
        for item in value:
            rewritten, item_count = _rewrite_chatflow_input_references(
                item,
                start_ids=start_ids,
            )
...
def _normalize_start_params(params: dict[str, Any], *, app_mode: str = "workflow") -> dict[str, Any]:
    ...
    for item in raw_variables:
        ...
        if app_mode == "advanced-chat" and variable["name"] in {"query", "files", "sys.query", "sys.files"}:
            continue
...
def _normalize_answer_params(params: dict[str, Any]) -> dict[str, Any]:
    answer = params.get("answer") or params.get("text") or params.get("content") or "{{#llm.text#}}"
    return {"answer": normalize_template_refs(str(answer))}
...
def _ensure_user_prompt(
    ...
    *,
    ...
    query_reference = "{{#sys.query#}}" if app_mode == "advanced-chat" else "{{#start.query#}}"
    if prompt:
        return f"{prompt}\n\n请处理以下用户输入：{query_reference}"
    return f"请根据以下用户输入完成{subject}任务：{query_reference}"
