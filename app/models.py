...
class PlanConversationVariable(BaseModel):
    id: str = ""
    name: str = Field(min_length=1)
    ...
    def validate_value(self) -> "PlanConversationVariable":
        if not _conversation_variable_value_matches(self.value_type, self.value):
            raise ValueError(
                f"conversation variable {self.name} has an invalid default for {self.value_type}"
            )
        return self
...
class WorkflowPlan(BaseModel):
    ...
    conversation_variables: list[PlanConversationVariable] = Field(default_factory=list)
    nodes: list[PlanNode]
    edges: list[PlanEdge]

    @model_validator(mode="after")
    def validate_graph(self) -> "WorkflowPlan":
        if self.app_mode != "advanced-chat" and self.conversation_variables:
            raise ValueError("conversation variables are only supported by advanced-chat")

        variable_names = [variable.name for variable in self.conversation_variables]
        if len(variable_names) != len(set(variable_names)):
            raise ValueError("conversation variable names must be unique")
        for variable in self.conversation_variables:
            if not variable.id:
                variable.id = str(
                    uuid5(
                        NAMESPACE_URL,
                        f"chat2dify:conversation-variable:{self.name}:{variable.name}",
                    )
                )
            try:
                UUID(variable.id)
                ...
                raise ValueError(
                    f"conversation variable {variable.name} must use a valid UUID"
                ) from exc
            variable.selector = ["conversation", variable.name]
        variable_ids = [variable.id for variable in self.conversation_variables]
        if len(variable_ids) != len(set(variable_ids)):
            raise ValueError("conversation variable ids must be unique")
...
def _conversation_variable_value_matches(value_type: str, value: Any) -> bool:
...
class CompletionRunDraftRequest(BaseModel):
    app_id: str = Field(min_length=1)
    query: str = ""
    inputs: dict[str, Any] = Field(default_factory=dict)
    files: list[dict[str, Any]] | None = None
...
class AgentRunDraftRequest(BaseModel):
    app_id: str = Field(min_length=1)
    query: str = Field(min_length=1)
    inputs: dict[str, Any] = Field(default_factory=dict)
    files: list[dict[str, Any]] | None = None
