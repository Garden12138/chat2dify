...
`knowledge-retrieval` retrieves context from the dataset IDs configured in
`DIFY_DEFAULT_DATASET_IDS`; `answer` remains out of scope for workflow mode.
...
Answer/chatflow nodes, tool/agent nodes, loops, and model capability registry
sync remain out of scope for now.
